export const WELCOMING_AD =
  "Relex and Unwind \nYour Cozy Retreat \nFrom Everyday Life";

export const REVIEWS = [
  {
    id: 1,
    review:
      "Impeccably clean rooms, friendly staff, and top-notch amenities. Highly recommended for a delightful stay!",
    guestName: "Sarah Thompson",
  },
  {
    id: 2,
    review:
      "Exceptional service from welcoming staff. Breathtaking ocean views and elegantly furnished rooms made for a memorable experience.",
    guestName: "The Smith Family",
  },
  {
    id: 3,
    review:
      "Tranquil atmosphere, rejuvenating spa, and cozy rooms. Minor Wi-Fi issue aside, it was a refreshing retreat.",
    guestName: "Emily and John",
  },
];

export const ABOUT_US =
  "Welcome to The Hotel where hospitality meets excellence. With 15 years of experience, we're dedicated to providing unparalleled service and memorable experiences. From our elegantly designed rooms to our world-class amenities, we strive to exceed expectations and create unforgettable stays for our guests.";

export const COMPANY_ADDRESS = "111 Some Street, Mountain View, CA 94040";

export const COMPANY_EMAIL = "email@gmail.com";

export const COMPANY_PHONE_NUMBER = "+1 (123) 456-7890";

export const COMPANY_FACEBOOK = "Facebook";

export const COMPANY_INSTAGRAM = "Instagram";

export const COMPANY_POLICY = "Privacy Policy & Cookies";

export const BOOKING_HEADING =
  "Welcome to The Hotel! Start planning your perfect stay with us.";
